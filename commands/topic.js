...
module.exports = {
    ...
    async execute(client, message, args, p) {
        ...
        if (!name) return message.channel.send("<:x_:755873672954380298> | Enter the name of the topic");
 
        message.channel.setTopic(args.slice(0).join(' '))

        message.channel.send("<:check:755873671498956811> | Topic renamed successfully!").then(x => x.delete({ timeout: 10000 }));
    }
}