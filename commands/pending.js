...
module.exports = {
    ...
    aliases: ["pending"],
    description: "Pending a server suggestion",
    ...
    async execute(client, message, args, p) {
        ...
        try {
            ...
            const acceptEmbed = new Discord.MessageEmbed()
                ...
                .setTitle("__**Suggestion Pending**__")
                .setThumbnail(data.thumbnail.url)
                .setDescription(data.description)
                .setImage(images)
                .setColor("GOLD")
                .addField("Status (PENDING)", acceptSuggestionQuery || "Has no reason")
                .addField("Pending by", message.author)

            message.channel.send("<:check:755873671498956811> | Suggestion pending successfully!").then(x => x.delete({ timeout: 6000 }));
            ...
            user.send(`<:x_:755873672954380298> | Your suggestion has been pending by: **\`${message.author.tag}\`**\n**\`Reason: ${acceptSuggestionQuery || "Has no reason"}\`**!`).catch(error => message.channel.send("<:x_:755873672954380298> | The author of the suggestion has closed DM's, so I cannot tell you that your suggestion was pending."));
        } catch (err) {
